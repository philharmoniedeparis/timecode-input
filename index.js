(function() {
  ...
  class TimecodeInput extends HTMLInputElement {
    ...
    constructor() {
      ...
      const regexp_str = SEGMENTS.map((s) => {
    ...
    _onKeypress(evt) {
      ...
      if ((typeof key === "number" || typeof key === "string" && key.trim() !== "") && !isNaN(key)) {
        // Numeric key.
        if (this._state.focused_segment < SEGMENTS.length) {
          ...
          segment_value =
            Math.min(
              SEGMENTS[this._state.focused_segment].max,
    ...
    _setSegmentValue(index, value) {
      ...
      if (matches) {
        ...
        matches.forEach((match, i) => {
          ...
          textual_value +=
            i === index ? value : matches[i] === this._options.placeholder ? "00" : matches[i];
        });
    ...
    _incrementSegmentValue(index) {
      this.value += SEGMENTS[index].multiplier;
      this._updateSelection();
    }
    ...
    _getTextualValue(value) {
      ...
      SEGMENTS.forEach(({ prefix, multiplier, max }) => {
  ...
  customElements.define("timecode-input", TimecodeInput, { extends: "input" });
})();